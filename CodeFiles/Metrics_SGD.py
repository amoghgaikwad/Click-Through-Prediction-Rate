...
Accuracy = labelsAndPreds.filter(lambda (v, p): v == p).count() / float(test.count())
print("Accuracy = " + str(Accuracy))

# 2- To Find the FPR
fpr_of_data= labelsAndPreds.filter(lambda (v, p): v==1 and p==0).count()
fpr= fpr_of_data/(fpr_of_data+labelsAndPreds.filter(lambda (v, p): v==0 and p==0).count())
print("FPR = " + str(fpr))
...
# We can also find the Area under precision-recall curve:
